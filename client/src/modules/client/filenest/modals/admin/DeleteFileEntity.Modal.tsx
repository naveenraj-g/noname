...
import { deleteFileEntity } from "../../server-actions/file-entity-action";

export const DeleteFileEntityModal = () => {
  ...
  const fileEntityDataId = useFilenestAdminStoreModal(
    (state) => state.fileEntityDataId
  );

  const isModalOpen = isOpen && modalType === "deleteFileEntity";

  const { execute, isPending } = useServerAction(deleteFileEntity, {
    onSuccess({ data }) {
      toast.success(`${data?.name ?? ""} file entity Deleted.`);
      handleCloseModal();
    },
    onError({ err }) {
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={handleCloseModal}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Delete File Entity</DialogTitle>
          <DialogDescription>
